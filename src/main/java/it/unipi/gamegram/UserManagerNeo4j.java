...
public class UserManagerNeo4j {
        ...
        try (Session session = Neo4jDbManager.getDriver().session()) {
            session.readTransaction(tx -> {
                Result result = tx.run("MATCH (username:User)-[:FOLLOW]-(followed) " +
                        ...
                        "AND username.username = " + usr.getEmail() +
                        "RETURN followed");
        ...
        if(checkIfFollowed(usrFrom, usrTo)) // you are already following this user
            ...
            try(Session session= neo4jDBM.getDriver().session()){
                ...
                session.writeTransaction((TransactionWork<Void>) tx -> {
                    tx.run ("CREATE (" + usrFrom.getEmail() +
                            ")-[:FOLLOW {date: date()}]->" +
                            "(" + usrTo.getEmail() +")");
                    return null;
                } );
    ...
    public boolean checkIfFollowed(User usrFrom, User usrTo){
        try(Session session= neo4jDBM.getDriver().session()){
            ...
            bool = session.readTransaction(tx -> {
                Result result = tx.run("MATCH (" + usrFrom.getEmail() + ":User)" +
                        " RETURN exists((" + usrFrom.getEmail() + ")-[:FOLLOW]->(" + usrTo.getEmail() + "))");
    ...
    public void addUserNode(User usr){
        try(Session session= neo4jDBM.getDriver().session()){
            session.writeTransaction((TransactionWork<Void>) tx -> {
                tx.run ("CREATE (" + usr.getEmail() + ":User { firstname:" + usr.getFirstName() + ", lastname:" + usr.getLastName() + "lastname, username:" + usr.getEmail() + "})");
                return null;
            } );
        }catch(Exception e){
            e.printStackTrace();
